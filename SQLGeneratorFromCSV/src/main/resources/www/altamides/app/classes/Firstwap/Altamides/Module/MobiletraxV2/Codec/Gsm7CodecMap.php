...
class Gsm7CodecMap
{
    ...
    public function decodeToStringBinary($string)
    {
        ...
        if($encoding !== self::TEXT_ENCODING){
            $string = mb_convert_encoding($string, self::TEXT_ENCODING);
        }
        
        $length    = mb_strlen($string, self::TEXT_ENCODING);
        $stringBin = '';
        for ($i = 0; $i < $length; $i++) {
            try {
                $char      = mb_substr($string, $i, 1, self::TEXT_ENCODING);
