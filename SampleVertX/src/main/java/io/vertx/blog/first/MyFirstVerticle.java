...
public class MyFirstVerticle extends AbstractVerticle
{
    ...
    private static final String ENDPOINT_EDIT = "/api/whiskies/:id";
    ...
    private void startWebApp(Handler<AsyncResult<HttpServer>> next)
    {
        ...
        router.get(ENDPOINT_EDIT).handler(this::getOne);
        router.put(ENDPOINT_EDIT).handler(this::updateOne);
        router.delete(ENDPOINT_EDIT).handler(this::deleteOne);
    ...
    private void updateOne(RoutingContext routingContext)
    {
        ...
        {
            jdbc.getConnection(ar -> update(id, json, ar.result(),
                    (whisky) -> {
                        if (whisky.failed())
                        {
                            routingContext.response().setStatusCode(404).end();
    ...
    private void update(String id, JsonObject content, SQLConnection connection, Handler<AsyncResult<Whisky>> resultHandler)
    {
        String sql = "UPDATE Whisky SET name=?, origin=? WHERE id=?";
        connection.updateWithParams(sql, new JsonArray().add(content.getString("name")).add(content.getString("origin")).add(id),
                update -> {
                    if (update.failed())
                    {
                    ...
                    resultHandler.handle(Future.succeededFuture(new Whisky(Integer.parseInt(id), content.getString("name"), content.getString("origin"))));
                });
    }
