...
public class JdbcTodoService implements TodoService
{
    ...
    {
        return client.rxUpdateWithParams(SQL_DELETE, new JsonArray().add(todoId)).ignoreElement();
    }

    @Override
    public Completable deleteAll()
    {
        return client.rxUpdate(SQL_DELETE_ALL).ignoreElement();
    }
    ...
    private static final String SQL_INSERT = "INSERT INTO `todo` (`id`, `title`, `completed`, `order`, `url`) VALUES (?, ?, ?, ?, ?)";
    ...
    private static final String SQL_UPDATE = "UPDATE `todo` SET `id` = ?, `title` = ?, `completed` = ?, `order` = ?, `url` = ? WHERE `id` = ?;";
